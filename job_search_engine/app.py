...
_TIER_BORING = re.compile("^(first |last |full |preferred )?(legal )?name$|^email|^phone|^resume|^cv$|^cover letter$|^linkedin|^website|^portfolio|^location$|^address|^city$|^pronouns$|^other$|profile url$|^how did you (first )?hear|referred|^gender|^race|^veteran|^disability|^ethnicity|^some other race|origin$|first nations|alaska native|^do you have a disability or chronic condition|^i (?:don't wish|prefer not)|decline to (?:self.identify|answer)|.*\\(\\d+ jobs?\\)$", re.I)
_TIER_MECHANICAL = re.compile("legally authoriz|work authoriz|sponsor|visa|18 years|age\\?|residing in|located within|currently authorized|privacy notice|country of residence|non-contractor|citizen|acknowledge|salary|compensation expectation|desired base|start date|notice period|eastern or pacific|are you open to|have you previously|legal address|street address|mailing address|^address", re.I)
_TIER_GATE = re.compile("relocat|in.person|in office|onsite|on-site|\\b\\w+ hubs?\\b|within \\d+ ?miles|days ?/ ?week|days a week|metropolitan area|willing to travel|travel \\d|hybrid|\\bbased (?:out of|in|near)\\b|commut|select locations?|open to candidates in|where (?:are|do) you (?:currently )?(?:live|located|reside)|\\b(?:NYC|SF|[A-Z][a-z]+, ?(?-i:AL|AK|AZ|AR|CA|CO|CT|DE|FL|GA|HI|ID|IL|IN|IA|KS|KY|LA|ME|MD|MA|MI|MN|MS|MO|MT|NE|NV|NH|NJ|NM|NY|NC|ND|OH|OK|OR|PA|RI|SC|SD|TN|TX|UT|VT|VA|WA|WV|WI|WY|DC))\\b|sponsor|visa|work permit|legally (?:authoriz|permitted|able|eligible|entitled)|(?:currently )?(?:eligible|authoriz\\w*) to (?:legally )?work|(?:legal )?right to work|work(?:ing)? rights|require .{0,30}authoriz|employment authoriz|on.call (?:schedule|rotation|duties|responsibilit)|rotating .{0,20}(?:schedule|shift)|\\bshift\\b|which of the following (?:locations|states)|live in one of the following|permanent residence|dual employment|non.compete|pacific (?:standard )?time|central (?:standard )?time|\\bPST\\b|\\bCST\\b|UTC[+-]\\d|scheduled shifts?|availability to work|willing to do|permanently reside|primary location|where will you be working|unrestricted work|background check|fingerprint|citizen or resident of|contract position|do you .{0,20}reside|primarily located|intend to work|evening shifts?|weekend shifts?|\\bBST\\b|\\bGMT\\b|\\bIST\\b|\\bET\\b\\b|employed at .{0,30} in any capacity|visiting facilit|visit (?:client|customer) site", re.I)
